...
const ProductDetailSection = ({products, addToBasket, fetchBasket}) => {
    ...
    const set_variant = (attribute_id, value_id) => {
        ...
        const filt = product.variants.filter(elem => elem.attributes_values[0].odoo_id == value_id);
        if (filt.length) {
                ...
                <div className='short-images'>
                    {
                        ...
                        variant.images.slice(1, 5).map((elem, id) => (
                            <img key={id} src={generate_image_full_path(variant.images[id + 1])} />
                        )) : null
                    }
                </div>
            ...
            <div className='product-descritpion'>
                <div className='description-content'>
                                ...
                                product.attributes.map((elem, id) => {
                                    return (
                                        <div className='attribute-elem' key={id}>
                                            <span className='select-title'>{elem.name}</span>
                                            <div className='select-container'>
                                                <select onInput={(e) => set_variant(elem.odoo_id, e.target.value)}>
                                                    {
                                                        elem.values.map((value, id) => (
                                                            <option key={id} value={value.odoo_id}>{value.name}</option>
                                                        ))
                                                    }
                                                </select>
                                            </div>
                    ...
                    <div className='price-container'>
                        <span className="price">{variant.list_price.toFixed(2)}€</span>
                        <span>TVAC</span>
                    </div>
                    ...
                    <div onClick={() => add_to_basket({image: generate_image_full_path(variant.images[0]), title: product.title, id: variant.odoo_id})} className={`primary add-basket-btn ${loading ? "loading" : ""}`}>
                        <img src={bag} />
...
const ProductDetail = ({products, addToBasket, fetchBasket}) => {
    return (
        <div id="product-detail">
            <ProductDetailSection products={products} addToBasket={addToBasket} fetchBasket={fetchBasket} />
            <ProductSection/>
            <OfferSection/>
            <CommitmentSection/>
            <TestimoniesSection/>
